...
def main():
    ...
    # Nessa linha estou definindo a resolução do 
    # "Monitor simulado" (15x15 pixels)
    largura, altura = 15, 15
    
    # Vou criar uma buffer, uma especie de Matriz de Pixels 
    # preenchida com zeros, que define a cor preta
    framebuffer = np.zeros((altura, largura))
    
    # Vou montar os pontos para definir a posição vetorial 
    # da linha. Ponto inicial e final geométricos
    x0, y0 = 2, 2
    x1, y1 = 12, 9
    
    # Aqui vai uma "simulação" BEEEEEEMMMMMMMMMM simplificada
    # do processo proposto por Bresenham
    ...
    # Só pra ficar legal a apresentação, vou colocar as 
    # duas imagens lado a lado. Criando uma figura lado a lado 
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
    fig.patch.set_facecolor('#1e1e1e') # cor de fundo
    ...
    # Nesse ponto é desenhada uma linha matematicamente perfeita (flutuante)
    ...
    # O cmap='gray' converte os 0s e 1s da matriz calculada acima 
    ax2.imshow(framebuffer, cmap='gray', 
               origin='lower', extent=[0, largura, 0, altura])
